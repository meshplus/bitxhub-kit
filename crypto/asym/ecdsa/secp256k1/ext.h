...
static secp256k1_context* secp256k1_context_create_sign_verify() {
	return bxh_secp256k1_context_create(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
}
...
static int secp256k1_ext_ecdsa_recover(
	...
	if (!bxh_secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &sig, sigdata, (int)sigdata[64])) {
		return 0;
	}
	if (!bxh_secp256k1_ecdsa_recover(ctx, &pubkey, &sig, msgdata)) {
		return 0;
	}
	size_t outputlen = 65;
	return bxh_secp256k1_ec_pubkey_serialize(ctx, pubkey_out, &outputlen, &pubkey, SECP256K1_EC_UNCOMPRESSED);
}
...
static int secp256k1_ext_ecdsa_verify(
	...
	return bxh_secp256k1_ecdsa_verify(ctx, &sig, msgdata, &pubkey);
}
