...
func gasSStoreEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	var (
		...
		current = evm.StateDB.GetEVMState(contract.Address(), slot)
		cost    = uint64(0)
	)
	// Check slot presence in the access list
	if addrPresent, slotPresent := evm.StateDB.SlotInEVMAceessList(contract.Address(), slot); !slotPresent {
		cost = ColdSloadCostEIP2929
		// If the caller cannot afford the cost, this change will be rolled back
		evm.StateDB.AddSlotToEVMAccessList(contract.Address(), slot)
		if !addrPresent {
	...
	original := evm.StateDB.GetEVMCommittedState(contract.Address(), x.Bytes32())
	if original == current {
		if original == (common.Hash{}) { // create slot (2.1.1)
			return cost + params.SstoreSetGasEIP2200, nil
		}
		if value == (common.Hash{}) { // delete slot (2.1.2b)
			evm.StateDB.AddEVMRefund(params.SstoreClearsScheduleRefundEIP2200)
		}
		// EIP-2200 original clause:
	...
	if original != (common.Hash{}) {
		if current == (common.Hash{}) { // recreate slot (2.2.1.1)
			evm.StateDB.SubEVMRefund(params.SstoreClearsScheduleRefundEIP2200)
	...
	if original == value {
		if original == (common.Hash{}) { // reset to original inexistent slot (2.2.2.1)
			...
			evm.StateDB.AddEVMRefund(params.SstoreSetGasEIP2200 - WarmStorageReadCostEIP2929)
			...
			evm.StateDB.AddEVMRefund((params.SstoreResetGasEIP2200 - ColdSloadCostEIP2929) - WarmStorageReadCostEIP2929)
		}
	}
	// EIP-2200 original clause:
...
func gasSLoadEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// Check slot presence in the access list
	if _, slotPresent := evm.StateDB.SlotInEVMAceessList(contract.Address(), slot); !slotPresent {
		// If the caller cannot afford the cost, this change will be rolled back
...
func gasExtCodeCopyEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// Check slot presence in the access list
	if !evm.StateDB.AddressInEVMAccessList(addr) {
		evm.StateDB.AddAddressToEVMAccessList(addr)
		var overflow bool
...
func gasSelfdestructEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if !evm.StateDB.AddressInEVMAccessList(address) {
		// If the caller cannot afford the cost, this change will be rolled back
		evm.StateDB.AddAddressToEVMAccessList(address)
		gas = ColdAccountAccessCostEIP2929
	}
	// if empty and transfers value
	if evm.StateDB.EmptyEVM(address) && evm.StateDB.GetEVMBalance(contract.Address()).Sign() != 0 {
